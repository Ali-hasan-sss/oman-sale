    ...
    <div className="relative inline-flex items-center gap-3">
      <button
        ...
        className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-bold transition ${
          data.userReaction ? 'border-brand-200 bg-brand-50 text-brand-700' : 'border-slate-200 bg-white text-slate-700 hover:bg-slate-50'
        }`}
      >
      ...
      {open ? (
        <div className="absolute start-0 top-full z-20 mt-2 flex gap-1 rounded-2xl border border-slate-200 bg-white p-2 shadow-lg">
          {REACTION_ORDER.map((type) => (
            <button
              ...
              className={`rounded-xl px-2 py-1 text-2xl transition hover:scale-110 ${
