...
import { Camera, ChevronDown, ChevronUp, Globe, Search, Store, Trash2, Upload, X } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir={dir}>
      <header className="sticky top-0 z-50 bg-white shadow-sm">
        <div className="mx-auto max-w-7xl px-4 py-4">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder={m.home.searchPlaceholder} className={`w-full rounded-lg border border-gray-300 py-3 outline-none focus:ring-2 focus:ring-green-500 ${dir === 'rtl' ? 'pl-4 pr-12' : 'pl-12 pr-4'}`} />
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-4 py-10">
        ...
        {isLoading ? (
          <div className="rounded-3xl bg-white p-10 text-center font-bold text-gray-500 shadow-sm">...</div>
        ) : !store ? (
            ...
            <form onSubmit={saveStore} className="space-y-6">
              <div className="overflow-hidden rounded-3xl bg-white shadow-sm">
                <div className="relative h-48 bg-slate-100">
                  ...
                  <label className="absolute bottom-4 right-4 inline-flex cursor-pointer items-center gap-2 rounded-xl bg-white/95 px-4 py-2 text-sm font-bold shadow">
                    <Upload size={16} />
                    {text.changeCover}
                    <input type="file" accept="image/*" onChange={(event) => uploadImage(event, 'cover')} className="hidden" />
                  </label>
                  {coverUrl ? (
                    <button type="button" onClick={() => setCoverUrl(null)} className="absolute bottom-4 left-4 rounded-xl bg-white/95 px-4 py-2 text-sm font-bold shadow">
                      {text.removeCover}
                    </button>
                  ) : null}
                </div>
