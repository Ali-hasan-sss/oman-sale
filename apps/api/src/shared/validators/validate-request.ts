...
export const validateRequest =
  (schemas: RequestSchemas) => (req: Request, _res: Response, next: NextFunction) => {
    ...
    if (body && !body.success) {
      return next(new ApiError(422, 'Invalid request body', ErrorCodes.VALIDATION_FAILED, body.error));
    }
    if (body) req.body = body.data as typeof req.body;

    const params = schemas.params?.safeParse(req.params);
    if (params && !params.success) {
      return next(new ApiError(422, 'Invalid route params', ErrorCodes.VALIDATION_FAILED, params.error));
    }
    if (params) req.params = params.data as typeof req.params;

    const query = schemas.query?.safeParse(req.query);
    if (query && !query.success) {
      return next(new ApiError(422, 'Invalid query params', ErrorCodes.VALIDATION_FAILED, query.error));
    }
